...
def EvalOneShotMetaInference(agent, env, reservoir,
                             rounds: int = 1,
                             lr_list: list[int] = (0.01, 0.03, 0.05, 0.1, 0.5, 0.8, 1, 3, 5, 10),
                             episodes: int = 600,
                             time_steps: int = 30,
    ...
    for lr in tqdm(lr_list, desc="k values", disable=not bar):
        ...
        if parallel:
            with ProcessPoolExecutor(max_workers=5) as pool:
                partial_eval = partial(
                    ...
                    k=1, rounds=rounds, episodes=episodes,
                ...
                for fut in tqdm(as_completed(futures), total=n_resets,
                                disable=not bar, desc=f'Eval lr={lr}'):
...
def main():
    ...
    data = EvalOneShotMetaInference(
        ...
        store_raw=True, 
        mode="no_entropic",
        outfile="one_shot_meta_inference_multiplier.json"
    )

    plot_one_shot_eval(lr_list, data, plotlog=True, savefig=True,
                       filename="one_shot_meta_inference_multiplier.png")
